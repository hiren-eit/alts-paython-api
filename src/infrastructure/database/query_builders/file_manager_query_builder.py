...
class DocumentManagerQueryBuilder:
    ...
    def _apply_user_filters(self):
            ...
            for text in f.search_text:
                ...
                search_conditions.append(
                    or_(
                        ...
                        # FileManager.tag.ilike(pattern) # tag not in new model?
                    )
                )
        ...
        if f.filter_created_date_from:
            query = query.filter(func.date(FileManager.createdate) >= f.filter_created_date_from)
        if f.filter_created_date_to:
            query = query.filter(func.date(FileManager.createdate) <= f.filter_created_date_to)
