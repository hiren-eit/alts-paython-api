...
class GetAllActiveFileListQueryBuilder:
    def __init__(
        self, db: Session, schematype: str | None = None, fileid: int | None = None
    ):
        self.db = db
        ...
        if self.schematype:
            ...
            self._query = (
                ...
                .filter(
                    ...
                    FileConfiguration.isactive
                    == True  # Ensures the record is active
                )
                .order_by(FileConfiguration.fileid.desc())
            )  # Order by Id DESC
        return self
    ...
    def build_query_master_config_type(self):
        """Build the query"""
        self._query = (
            self.db.query(MasterConfigurationType)
            .filter(
                MasterConfigurationType.type == "ContentType",
                MasterConfigurationType.isactive == True,
            )
            .order_by(MasterConfigurationType.masterconfigurationtypeid)
        )  # Order by Id DESC
        return self
